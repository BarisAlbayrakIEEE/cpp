...
namespace GeometryNamespace {
	...
	/// </summary>
	VectorBase VectorBase::operator+(ARGCOPY(VectorBase) theVector) {
	...
	/// <summary>
	/// Downcast from Point2D to PointBase
	/// </summary>
	VectorBase VectorBase::DownCast(ARGCOPY(Vector2D) theVector) {
		return VectorBase(
			DIMENSIONS::D2,
			theVector.getReferenceCoordSystem(),
			theVector.getLocalComponents());
	}

	/// <summary>
	/// Downcast from Point3D to PointBase
	/// </summary>
	VectorBase VectorBase::DownCast(ARGCOPY(Vector3D) theVector) {
		return VectorBase(
			DIMENSIONS::D3,
			theVector.getReferenceCoordSystem(),
			theVector.getLocalComponents());
	...
	/// Used in the copy/move ctor and operators
	/// </summary>
	VectorBase VectorBase::clone(ARGCOPY(VectorBase) theVector)
	{
		return VectorBase(
			theVector.getDimensionCount(),
			theVector.getReferenceCoordSystem(),
			theVector.getLocalComponents());
	}
	...
	/// </summary>
	void VectorBase::setMembers(int theDimensionCount, CoordSystem& theCoordSystem, const arrayS3& theLocalComponents) {
	...
	/// Protected method used in this class and child classes only.
	/// </summary>
	void VectorBase::setMembers(int theDimensionCount, CoordSystem& theCoordSystem, const vectorInput1D& theLocalComponents) {
		ReferenceObject::setMembers(theDimensionCount, theCoordSystem);
		setLocalComponents(theLocalComponents);
	}
		...
		bool theKeepGlobalComponentsSame)
	{
		...
		if (theCoordSystem.isGlobal())
		{
			...
			Point3D point{ Point3D(*GlobalCoordSystem::getGlobalCoordSystem(), getGlobalComponents()) };
	...
	{
		arrayS3 localComponents0{ getGlobalComponents() };
		arrayS3 localComponents1{ theVector.getGlobalComponents() };
	...
	/// <exception> NullptrException </exception>
	/// <exception> ZeroVectorException </exception>
	Vector3D VectorBase::crossProduct(ARGCOPY(VectorBase) theVector) const
	{
		arrayS3 localComponents0 = getGlobalComponents();
		arrayS3 localComponents1 = getVectorWithMyCoordSystem(theVector).getGlobalComponents();
		...
		if (GeometryMath::equalsZero<double, 3>(vectorComponents, TOLERANCE_GENERAL)) {
			throw ZeroVectorException();
		}

		return Vector3D(vectorComponents);
	}
	...
	/// </summary>
	/// <exception> NullptrException </exception>
	Point3D VectorBase::transformPoint(
		...
		const double& theFactor) const
	{
		...
		return Point3D(*c_referenceCoordSystem, finalCoords);
	}
	...
	/// <exception> NullptrException </exception>
	/// <exception> ZeroVectorException </exception>
	VectorBase VectorBase::add(ARGCOPY(VectorBase) theVector) const
	{
		// Get the item in my reference CS
		arrayS3 localCoords{ GeometryMath::sumArrays(c_localComponents, getVectorWithMyCoordSystem(theVector).getLocalComponents()) };
		try
		{
			if (is2D() && GeometryMath::equals(localCoords[2], 0., c_toleranceGeneral))
			{
				return VectorBase(DIMENSIONS::D2, *c_referenceCoordSystem, localCoords);
			}
			return VectorBase(DIMENSIONS::D3, *c_referenceCoordSystem, localCoords);
		}
		catch (ZeroVectorException)
		{
			throw;
	...
	/// <exception> NullptrException </exception>
	/// <exception> ZeroVectorException </exception>
	VectorBase VectorBase::subtruct(ARGCOPY(VectorBase) theVector) const
	{
		// Get the item in my reference CS
		arrayS3 localCoords{ GeometryMath::subtructArrays(c_localComponents, getVectorWithMyCoordSystem(theVector).getLocalComponents()) };
		try
		{
			if (is2D() && GeometryMath::equals(localCoords[2], 0., c_toleranceGeneral))
			{
	...
	/// </summary>
	/// <exception> ZeroVectorException </exception>
	VectorBase VectorBase::multiply(const double& theFactor) const
	{
		...
		try
		{
			...
			{
				return VectorBase(DIMENSIONS::D2, *c_referenceCoordSystem, finalCoords);
			}
			return VectorBase(DIMENSIONS::D3, *c_referenceCoordSystem, finalCoords);
		}
		catch (ZeroVectorException)
		{
			throw;
	...
	/// </summary>
	VectorBase VectorBase::createUnitVectorX(int theDimensionCount) {
		return VectorBase(theDimensionCount, *GlobalCoordSystem::getGlobalCoordSystem(), arrayS3{ 1., 0., 0. });
	}

	/// <summary>
	/// Create unit vector - Global- Y
	/// </summary>
	VectorBase VectorBase::createUnitVectorY(int theDimensionCount)
	{
		return VectorBase(theDimensionCount, *GlobalCoordSystem::getGlobalCoordSystem(), arrayS3{ 0., 1., 0. });
	}

	/// <summary>
	/// Create unit vector - Global- Z
	/// </summary>
	Vector3D VectorBase::createUnitVectorZ()
	{
		return Vector3D(*GlobalCoordSystem::getGlobalCoordSystem(), arrayS3{ 0., 0., 1. });
	}

	/// <summary>
	/// Create unit vector - Input CS - X
	/// </summary>
	VectorBase VectorBase::createUnitVectorX(int theDimensionCount, CoordSystem& theCoordSystem)
	{
		//return new VectorBase(theDimensionCount, theCoordSystem, arrayS3{ 1., 0., 0. });
		if (theDimensionCount == DIMENSIONS::D2)
		{
			return VectorBase(DIMENSIONS::D2, theCoordSystem, arrayS3{ 1., 0., 0. });
		}
		return VectorBase(DIMENSIONS::D3, theCoordSystem, arrayS3{ 1., 0., 0. });
	}

	/// <summary>
	/// Create unit vector - Input CS - Y
	/// </summary>
	VectorBase VectorBase::createUnitVectorY(int theDimensionCount, CoordSystem& theCoordSystem)
	{
		return VectorBase(theDimensionCount, theCoordSystem, arrayS3{ 0., 1., 0. });
	}

	/// <summary>
	/// Create unit vector - Input CS - Z
	/// </summary>
	Vector3D VectorBase::createUnitVectorZ(CoordSystem& theCoordSystem)
	{
		Vector3D vector { Vector3D(theCoordSystem, arrayS3{ 0., 0., 1. }) };
		return vector;
	}
